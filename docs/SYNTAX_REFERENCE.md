...
- **Тег** — завершающий атом строки: `#` без пробела, далее идентификатор,
  ...
  поиска, исходное написание сохраняется. Теги никогда не меняют ответ строки
...
`;` в запятых). `rand(low, high)` — включительно, равномерно, из системной
энтропии. Строка, зависящая от `rand`, помечается динамической: её значение
стабильно в течение эпохи (повторные перерисовки не меняют его), но такое
значение НЕ токенизируется и не участвует в цепочке предыдущего ответа.
`⌘R` (Recalculate Dynamic Values) начинает новую эпоху.
