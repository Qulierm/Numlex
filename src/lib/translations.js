// Interface translations, ported from the legacy src/translations.js.

export const translations = {
	en: {
		sheet: 'Sheet',
		newSheet: 'New sheet',
		deleteSheet: 'Delete sheet',
		sheets: 'Sheets',
		showSheets: 'Show sheets',
		hideSheets: 'Hide sheets',
		importSheet: 'Import sheet (.nlx)',
		exportSheet: 'Export sheet (.nlx)',
		enter: 'Enter an expression to start',
		settings: 'Settings',
		sumOfResults: 'Sum of results',
		round: 'Rounding numbers:',
		fontcolor: 'Font color:',
		blue: 'Blue',
		purple: 'Purple',
		red: 'Red',
		green: 'Green',
		white: 'White',
		fontsize: 'Font size:',
		sheetname: 'Sheet title:',
		language: 'Interface language:',
		linenumber: 'Line numbers:',
	},
	ru: {
		sheet: 'Лист',
		newSheet: 'Новый лист',
		deleteSheet: 'Удалить лист',
		sheets: 'Листы',
		showSheets: 'Показать листы',
		hideSheets: 'Скрыть листы',
		importSheet: 'Импорт листа (.nlx)',
		exportSheet: 'Экспорт листа (.nlx)',
		enter: 'Введите выражение чтобы начать',
		settings: 'Настройки',
		sumOfResults: 'Сумма результатов',
		round: 'Округление чисел:',
		fontcolor: 'Цвет текста:',
		blue: 'Синий',
		purple: 'Фиолетовый',
		red: 'Красный',
		green: 'Зеленый',
		white: 'Белый',
		fontsize: 'Размер текста:',
		sheetname: 'Заголовок листов:',
		language: 'Язык интерфейса:',
		linenumber: 'Номера строк',
	},
	de: {
		sheet: 'Blatt',
		newSheet: 'Neues Blatt',
		deleteSheet: 'Blatt löschen',
		sheets: 'Blätter',
		showSheets: 'Blätter anzeigen',
		hideSheets: 'Blätter ausblenden',
		importSheet: 'Blatt importieren (.nlx)',
		exportSheet: 'Blatt exportieren (.nlx)',
		enter: 'Geben Sie einen Ausdruck ein, um zu beginnen',
		settings: 'Einstellungen',
		sumOfResults: 'Summe der Ergebnisse',
		round: 'Rundung von Zahlen',
		fontcolor: 'Schriftfarbe:',
		blue: 'Blau',
		purple: 'Violett',
		red: 'Rot',
		green: 'Grün',
		white: 'Weiss',
		fontsize: 'Schriftgröße',
		sheetname: 'Titel der Blätter:',
		language: 'Sprache der Schnittstelle:',
		linenumber: 'Zeilennummern:',
	},
	fr: {
		sheet: 'Feuille',
		newSheet: 'Nouvelle feuille',
		deleteSheet: 'Supprimer la feuille',
		sheets: 'Feuilles',
		showSheets: 'Afficher les feuilles',
		hideSheets: 'Masquer les feuilles',
		importSheet: 'Importer une feuille (.nlx)',
		exportSheet: 'Exporter la feuille (.nlx)',
		enter: "Entrez une expression pour commencer",
		settings: 'Paramètres',
		sumOfResults: 'Somme des résultats',
		round: 'Arrondir les nombres:',
		fontcolor: 'Couleur de la police:',
		blue: 'Bleu',
		purple: 'Violet',
		red: 'Rouge',
		green: 'Vert',
		white: 'Blanc',
		fontsize: 'Taille de la police:',
		sheetname: 'Titre de la feuille:',
		language: "Langue de l'interface:",
		linenumber: 'Numéros de ligne:',
	},
	it: {
		sheet: 'Foglio',
		newSheet: 'Nuovo foglio',
		deleteSheet: 'Elimina foglio',
		sheets: 'Fogli',
		showSheets: 'Mostra fogli',
		hideSheets: 'Nascondi fogli',
		importSheet: 'Importa foglio (.nlx)',
		exportSheet: 'Esporta foglio (.nlx)',
		enter: "Inserisci un'espressione per iniziare",
		settings: 'Impostazioni',
		sumOfResults: 'Somma dei risultati',
		round: 'Arrotondamento numeri:',
		fontcolor: 'Colore del carattere:',
		blue: 'Blu',
		purple: 'Viola',
		red: 'Rosso',
		green: 'Verde',
		white: 'Bianco',
		fontsize: 'Dimensione del carattere:',
		sheetname: 'Titolo del foglio:',
		language: "Lingua dell'interfaccia:",
		linenumber: 'Numeri di riga:',
	},
	ch: {
		sheet: '工作表',
		newSheet: '新工作表',
		deleteSheet: '删除工作表',
		sheets: '工作表',
		showSheets: '显示工作表',
		hideSheets: '隐藏工作表',
		importSheet: '导入工作表 (.nlx)',
		exportSheet: '导出工作表 (.nlx)',
		enter: '输入表达式开始',
		settings: '设置',
		sumOfResults: '结果总和',
		round: '四舍五入的数字：',
		fontcolor: '字体颜色：',
		blue: '蓝色',
		purple: '紫色',
		red: '红色',
		green: '绿色',
		white: '白色',
		fontsize: '字体大小：',
		sheetname: '工作表标题：',
		language: '界面语言：',
		linenumber: '行号：',
	},
}

export const LANGUAGES = [
	{ id: 'en', value: 'en', label: 'English' },
	{ id: 'ru', value: 'ru', label: 'Русский' },
	{ id: 'de', value: 'de', label: 'Deutsch' },
	{ id: 'it', value: 'it', label: 'Italiano' },
	{ id: 'fr', value: 'fr', label: 'Français' },
	{ id: 'ch', value: 'ch', label: '中文' },
]
