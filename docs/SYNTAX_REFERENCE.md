...
- **Двойной клик по пустой строке ответа** (строка только из пробелов или
  ...
  остаются инертными.
- `grand total` (и `<имя> = grand total`) — сумма уже вычисленных успешных
  строк-подытогов выше по листу (разделитель не стирает список). Требуется
  минимум два подытога, иначе тихая ошибка.
