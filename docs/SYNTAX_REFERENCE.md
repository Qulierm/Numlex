...
CAGR `(returned/invested)^(1/y) − 1` (kind процента):

```text
annual return on $1,000 invested $1,500 returned after 3 years → 14.4714242553%
```
...
`set sales tax in Settings → Tax`. Пресеты (версия `tax-presets-2026.1`)
задают национальные стандартные ставки: AU GST 10, GB VAT 20, DE VAT 19,
NL VAT 21 и другие; у US автоматической ставки нет — только ручной ввод.
При ставке 15%:
...
Встроенный офлайн-набор `income-tax-2026.1` (10 стран: US, GB, AU, CA, DE,
FR, NL, IN, RU, JP), SHA-256 в манифесте, официальные источники и год
таблицы указаны для каждой страны. Модель: грубая национальная оценка —
...
CPI-U All Items (1982-84=100) с 1913 по последний полный год, плюс явно
